...
import {
  getSearchingData,
  getResultMessage,
} from "./services/SearchingDataSource";
...
<template>
  <section class="map">
      ...
      <div v-if="resultList.length > 0" class="searching-list-component">
        ...
        <div class="searching-list">
          <li v-for="item in resultList" :key="item.id">
            <div class="search-item" @click="handleItemOnClick(item)">
              <svg
                class="pin-icon"
                viewBox="0 0 11 14"
    ...
    <MapModal v-show="showModal" @created="handleCloseModal"/>
  </section>
</template>
<style>
.map {
